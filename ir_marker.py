...
#    out = order_ir_markers(ir)
    ...
    return order_ir_markers(ir, last_visible_markers) #Order the markers and return
...
def order_ir_markers(markers, last_visible_markers, offset_angle=135):
    ...
    if markers == None:
        ...
        for marker_perm in marker_perms:
            ...
            for marker, old_marker in zip(marker_perm, last_visible_markers):
                dist_sum += np.linalg.norm(np.array(marker["pos"]) - np.array(old_marker["pos"]))
                pprint(dist_sum)
